...
func TestCreateTestVendor(t *testing.T) {
	...
	if vendor.Category != Vendor {
		t.Errorf("Expected vendor category '%s', got '%s'", Vendor, vendor.Category)
	}

	// Verify vendor was saved to database
	var count int64
	db.Model(&Node{}).Where("id = ?", vendor.ID).Count(&count)
	if count != 1 {
...
func TestCreateTestProduct(t *testing.T) {
	...
	product := CreateTestProduct(t, db, "Test Product", "A test product", vendor.ID, Software)
	...
	if product.ProductType != Software {
		t.Errorf("Expected product type '%s', got '%s'", Software, product.ProductType)
	}
...
func TestCreateTestProductVersion(t *testing.T) {
	...
	vendor := CreateTestVendor(t, db, "Vendor", "Test vendor")
	product := CreateTestProduct(t, db, "Product", "Test product", vendor.ID, Software)
	...
	if version.Category != ProductVersion {
		t.Errorf("Expected version category '%s', got '%s'", ProductVersion, version.Category)
	}
...
func TestCreateTestRelationship(t *testing.T) {
	...
	relationship := CreateTestRelationship(t, db, version.ID, version.ID, DefaultComponentOf)
	...
	if relationship.Category != DefaultComponentOf {
		t.Errorf("Expected relationship category '%s', got '%s'", DefaultComponentOf, relationship.Category)
	}
}
